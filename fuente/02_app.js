...
function bloqueCorregir(r, regs){
  ...
  const puntos = [
    ...
    /* Se le abrió al ejecutivo el 24/08 —«es lo único de la llave que puede
       ...
       incentivo. Nadie debería poder editar la evidencia de su propia
       medición.

       Lo que SÍ sigue viendo, en «Tu registro al día», es el porcentaje contra
       su mínimo y cuántas gestiones a tiempo le faltan. Ese número es lo único
       que le permite corregir antes del 18 y no le deja tocar nada: es un
       conteo, no una lista de filas editables. Se le quita la puerta, no el
       espejo. */
...
/* ---- El corte de la semana cumplida ------------------------------------
   ...
   LA SEMANA CIERRA EL DOMINGO A LAS 23:59:59, y va del lunes al domingo.

   Esto se movió dos veces en cuatro días y conviene que quede escrito por qué,
   porque el motivo es el mismo las dos veces. El corte era el JUEVES, y el
   01/09 apareció el síntoma: la lámina semanal decía 1 retención donde el
   equipo había hecho 2. Las dos eran correctas y estaban registradas; lo que
   pasaba es que una se cerró el jueves 27/08 y ese jueves caía en la semana
   que ya se había cerrado, no en la que el comité estaba mirando.

   El día del cierre es un borde, y UN BORDE TIENE QUE CAER DONDE NADIE ESTÁ
   TRABAJANDO EL NÚMERO QUE SE PRESENTA. Primero se movió al miércoles; el
   mismo día José lo llevó al domingo, que es donde el borde de verdad no
   estorba: la semana de campo es de lunes a domingo, nadie cierra un comercio
   un domingo a las once de la noche, y el lunes empieza limpio.

   El corte es el último domingo ya cumplido —por eso se empieza a mirar desde
   ayer: si hoy es domingo, el día todavía no termina y pertenece a la semana
   en curso—. Y la semana en curso va del lunes siguiente HASTA HOY, que es el
   momento en que se descarga el archivo: ese es el comparativo que pidió José,
   el cierre de la semana pasada contra lo que va hasta la descarga. */
...
function repSemanaDia(f){
  ...
  /* La semana anterior va del lunes al domingo del corte; la actual arranca
     el lunes siguiente y llega hasta hoy —el momento en que se descarga el
     archivo—. Los dos arreglos tienen siete casillas para que el día 3 de una
     caiga sobre el día 3 de la otra.
...
/* Las tres series que mira el comité, semana a semana. Semanas de LUNES A
   DOMINGO —el corte es el domingo a las 23:59:59, ver `repCorteSemana`—; la
...
function repSemanal(f){
  ...
  /* Se arma hacia atrás desde el corte y se corta en el arranque: así el
     último domingo siempre cae en el borde de una semana y la comparación
  ...
  /* La semana en curso: del lunes siguiente al corte hasta HOY, que es el
     momento en que se descarga el archivo. */
...
function armarDirectorio(pptx, fondos){
  ...
  titulo(g, "El desarrollo, semana a semana",
    `Actividad de cada semana —no el acumulado— · la semana cierra el domingo a las 23:59 · última cumplida al ${repFechaLarga(SEM.corte)}`);
  ...
  SEM.series.forEach((serie, i2) => {
    ...
    /* El eje lleva fechas, no «D1» y «D7»: nadie tiene por qué traducir un día
       ...
       domingo del corte, a la derecha hoy —puesto bajo el nodo de hoy y no en el
